...
export default function VersionHistory({ documentId, currentContent, onClose, onRolledBack }) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-96 bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-850 shadow-xl flex flex-col z-40 text-gray-900 dark:text-gray-100">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-800">
        <h3 className="font-semibold text-sm">Version History</h3>
        <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-250 text-sm">✕</button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {versions?.map((v) => (
          <button
            ...
            className={`w-full text-left px-4 py-2.5 border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 bg-transparent ${
              v.id === selectedVersionId ? 'bg-indigo-50/50 dark:bg-indigo-950/40 text-indigo-900 dark:text-indigo-200' : 'text-gray-750 dark:text-gray-305'
            }`}
          >
      ...
      {selectedVersion && (
        <div className="border-t border-gray-200 dark:border-gray-800 p-4 max-h-80 overflow-y-auto bg-gray-50 dark:bg-gray-850">
          ...
          <div className="text-sm leading-relaxed whitespace-pre-wrap">
            {diffParts.map((part, i) => (
              <span
                key={i}
                className={
                  part.added ? 'bg-green-100 dark:bg-green-950/30 text-green-800 dark:text-green-300'
                    : part.removed ? 'bg-red-100 dark:bg-red-950/30 text-red-800 dark:text-red-300 line-through'
